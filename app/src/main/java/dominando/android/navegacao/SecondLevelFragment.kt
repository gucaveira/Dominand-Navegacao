...
class SecondLevelFragment : Fragment() {
    ...
    private val binding: FragmentSecondLevelBinding get() = _binding!!

    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ) = FragmentSecondLevelBinding.inflate(inflater, container, false).apply {
        _binding = this
    }.root

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        arguments?.getString(EXTRA_TEXT)?.let { binding.textView.text = it }
        arguments?.getInt(EXTRA_TEXT_COLOR)?.let { binding.textView.setTextColor(it) }
        arguments?.getInt(EXTRA_BG_COLOR)?.let { view.setBackgroundColor(it) }
    }
